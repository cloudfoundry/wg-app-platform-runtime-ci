...
if [[ -n "${BBL_STATE_DIR}" ]]; then
    ...
    DIRECTOR=$(jq -r .current_vm_cid < vars/director-state.json )
    JUMPBOX=$(jq -r .current_vm_cid < vars/jumpbox-state.json )
    gcloud compute instances stop "${DIRECTOR}" --project "${PROJECT}" --zone "${ZONE}"
    gcloud compute instances stop "${JUMPBOX}" --project "${PROJECT}" --zone "${ZONE}"
fi
