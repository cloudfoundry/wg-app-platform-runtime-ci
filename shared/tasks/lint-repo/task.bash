...
function run(){
    ...
    for linter in ${LINTERS}; do
        ...
        local repo_linter_alt_location="./ci-config/${repo_name}/linters/${linter}"
        if [[ -f "$repo_linter" ]]; then
            echo "Running $repo_linter for-$repo_name with-exit-on-error=true"
            "$repo_linter" "$PWD/repo" true
        elif [[ -f "$repo_linter_alt_location" ]]; then
            echo "Running $repo_linter_alt_location for-$repo_name with-exit-on-error=true"
            "$repo_linter_alt_location" "$PWD/repo" true
